...
index=$2
user=$3
echo "The recorded start of data collection is: $user"
echo "is $user the value you would like to use? [y/n]"

option=0
while [ $option -ne 5 ]
do
echo "Welcome to the SMTP meta data puller:"
...
read warning
    ...
    python3 ./Scripts/multi.py $node $index $user
    fi
fi
...
read warning
    ...
    python3 ./Scripts/past_n.py $node $index $days
    echo $user
...
read standard
    ...
    python3 ./Scripts/LFA.py "./MostRecentCollection/MostRecentCollection.json" './Results/LFA_logs.json' $percentage
fi

if [ $option -eq 4 ];
then
    echo "Creating .txt file output on the last completed LFA:"
    python3 ./Scripts/DisplayAllColumns.py "./Results/LFA_logs.json" "./Whitelist/"
