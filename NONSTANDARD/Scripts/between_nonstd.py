...
for key in non_std.keys():
    ...
    result = GREY_FUNCTIONS.nonstd_search(node, index, days, port_query, protocol_query, columns)
    if result:
        flag_txt =""
        if remove_anomoly:
            flag_txt = "_NO_ANOMOLY"
        dir_path = "./Results/" + "nonstd_Between_" + days[0]+days[-1] + flag_txt
    ...
    print("-"*70)
from os import walk 
filenames = next(walk("./TMP/"), (None, None, []))[2]  # [] if no file
