...
while [ $option -ne 5 ]
do
echo "Welcome to the Proxy meta data puller:"
...
read warning
    ...
    python3 ./Scripts/proxy_multi.py $node $index $user
    fi
fi
...
read warning
    ...
    python3 ./Scripts/past_n_proxy.py $node $index $days
    echo $user
    python3 ./Scripts/proxy@rest.py $node $index $user
    fi
fi
...
read standard
    ...
    python3 ./Scripts/LFA.py "./MostRecentCollection/MostRecentCollection.json" './Results/LFA_proxy_logs.json' $percentage
fi

if [ $option -eq 4 ];
then
    echo "Creating .txt file output on the last completed LFA:"
    python3 ./Scripts/DisplayAllColumns.py "./Results/LFA_proxy_logs.json" "./Whitelist/"
